...
// ─── Button ─────────────────────────────────────────────────────────
function RButton({ children, variant = 'solid', onClick, href, download, tone = RP.ink, small, style, type = 'button' }) {
  const base = {
  ...
  const props = href ? { href, ...(download !== undefined && { download }) } : { onClick, type };
  return (
...
function RefinedHeader({ page, onNav, lang, setLang, t }) {
  ...
  const navLabel = { home: t.navHome, portfolio: t.navPortfolio, music: t.navMusic, shows: t.navShows, press: t.navPress, about: t.navAbout, shop: t.navShop, work: t.navWork };
  return (
    <header style={{
